...
(** TESTING PLAN: We tested every function documented in Manual.mli, 
    ...
    init_task, init_todolist, and empty_list because they were all indirectly
     ...
     because it was a helper to several other functions that were tested.  
     to_list_app and to_list_find were also not tested because 
     they were used in the test file to create a meaningful representation
     of appointments to be compared with expected_output.  From Appointments.mli
     the two functions that were included in the mli file that were not tested 
     are to_list_app and to_list_find for the reasons previously stated above.


     We omitted testing helper functions, because they were
    tested by testing their parent functions. We tested various possibilities/
    branches/cases for each of our functions, and always made sure to test tasks
    from different categories to ensure that our mutable variable categories was 
    correctly storing the right information. 
