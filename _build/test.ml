...
(** TESTING PLAN: We tested every function documented in Manual.mli, 
    ...
    the automatic to-do list to be compared with expected_output.

    From Manual.mli, we omitted directly testing access_cat, empty_cat, 
    todays_date, init_task, init_todolist, and empty_list because they were all 
    indirectly tested with other functions. From Manual.mli, to_list was also 
    not tested because it was used in the test file to create a meaningful r
    epresentation of the manual to-do list to be compared with expected_output. 

    From Appointments.ml, the functions not tested were find_app, to_list_app, 
    and to_list_find.  Find_app was not tested because it was a helper to
    several other functions that were tested. to_list_app and to_list_find were
    also not tested because they were used in the test file to create a
    meaningful representation of appointments to be compared with
    expected_output.  From Appointments.mli, the two functions that were
    included in the mli file that were not tested are to_list_app and
    to_list_find for the reasons previously stated above.

    Gui.ml was tested manually by trying all the different functionality in
    the window. All functionality that is in the .mli files of Appointments,
    Automatic, and Manual is in the gui and it has all been tested. We feel
    confident that all of the functions in Appointments, Automatic, and Manual
    work because the tests done in this file and the manual testing in the 
    gui show that they appear the way we want them to in the gui.

    We omitted testing helper functions, because they were tested by testing
    their parent functions. We tested various possibilities/branches/cases for
    each of our functions, and always made sure to test tasks from different
    categories to ensure that our mutable variable categories was correctly
    storing the right information. 
...
let change_due_date_tests2 = 
  [
    update_cat_test "fill out OMM in School should now be due 10/27/20 and do 
    lab in school should now be due 10/25/20" cat 
      "School" ["School";  "do lab"; todays_date (); "10/25/20"; "3";
                "fill out OMM"; todays_date (); "10/27/20"; "1";
                "watch lecture"; todays_date (); "10/28/20"; "2"];
  ]
...
let change = change_due_auto ~cat:cat "Shopping Tasks"
    "Get New iPhone 12 Pro Max" "11/13/2020"
...
let cat = empty_cat_auto ()
let car = make_car_auto ~cat:cat ()
let change = change_name_auto ~cat:cat "Car Tasks" "Change Oil" "Change Tires" 
let reset = reset_car ~cat:cat ()

let reset_car_tests = [
  update_cat_test_auto "Reset car category after a change" cat "Car Tasks"
    [
      "Car Tasks";
...
let cat = empty_cat_auto ()
let school = make_school_auto ~cat:cat ()
let change = change_due_auto ~cat:cat "School Tasks" "Finish Biology Lab"
    "12/21/2020" 
let reset = reset_school ~cat:cat ()

let reset_school_tests = [
  update_cat_test_auto "Reset school category after a change" cat "School Tasks"
    [
      "School Tasks";
      "Plan for Pre-Enroll"; todays_date (); "TBD"; "7"; 
      "Watch CS 3110 Lecture Videos"; todays_date (); "TBD"; "6"; 
...
let cat = empty_cat_auto ()
let shopping = make_shopping_auto ~cat:cat ()
let change = change_priority_auto ~cat:cat "Shopping Tasks"
    "Find Dress for Formal" 2
let reset = reset_shopping ~cat:cat ()

let reset_shopping_tests = [
  update_cat_test_auto "Reset shopping category after a change" cat
    "Shopping Tasks"
    [
      "Shopping Tasks";
      "Get New iPhone 12 Pro Max"; todays_date (); "TBD"; "7";
...
let cat = empty_cat_auto ()
let pandemic = make_pandemic_auto ~cat:cat ()
let change = change_name_auto ~cat:cat "Pandemic Tasks" "Get Tested"
    "Schedule Test"
let reset = reset_pandemic ~cat:cat ()

let reset_pandemic_tests = [
  update_cat_test_auto "Reset pandemic category after a change" cat
    "Pandemic Tasks"
    [
      "Pandemic Tasks";
      "Cancel Flights"; todays_date (); "TBD"; "7";
...
let meeting2 = add_app_info ~appo:apps "Staff Meeting" 
    "meeting to determine third financial quarter goals"
let report2 = add_app_info ~appo:apps "Company report due" 
    "report needs to be 7-10 pages long"
let lunch2 = add_app_info ~appo:apps "Lunch with Client" 
...
let suite =
  "test suite for all functionality"  >::: List.flatten [
    ...
    reset_car_tests;
    reset_school_tests;
    reset_household_tests;
    reset_shopping_tests;
    reset_pandemic_tests;
    reset_all_cat_tests;
